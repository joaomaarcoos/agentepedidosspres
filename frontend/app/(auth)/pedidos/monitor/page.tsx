...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span style={{ padding: "3px 12px", borderRadius: 20, background: s.bg, color: s.color, border: `1px solid ${s.color}`, fontSize: 11, fontWeight: 700, textTransform: "uppercase" }}>
      {s.label}
    </span>
  );
}
        ...
        <div style={{ padding: "16px 24px", borderTop: "1px solid var(--border)", background: "rgba(0,0,0,0.15)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            {Object.keys(statusBreakdown).length > 0 && (
              <div>
                <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 8, textTransform: "uppercase", letterSpacing: 1 }}>Breakdown por situação</div>
                {Object.entries(statusBreakdown).map(([k, v]) => (
                  <div key={k} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--text)", padding: "3px 0" }}>
                    <span>{k}</span>
...
export default function MonitorPage() {
  ...
  const loadLogs = useCallback(async () => {
    setLoading(true);
    try {
      const r = await pedidosApi.getSyncLogs(date);
      setLogs(r.logs);
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, overflow: "auto", padding: 28 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
          ...
          <input type="date" value={date} onChange={(e) => setDate(e.target.value)} style={{ background: "var(--surface2)", color: "var(--text)", border: "1px solid var(--border)", borderRadius: 8, padding: "7px 12px", fontSize: 13 }} />
          <button onClick={loadLogs} disabled={loading} style={{ display: "flex", alignItems: "center", gap: 7, background: "var(--surface2)", color: "var(--text)", border: "1px solid var(--border)", borderRadius: 8, padding: "8px 14px", fontSize: 13, cursor: loading ? "not-allowed" : "pointer" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 24 }}>
          {[
            ...
            <div key={label} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: "14px 18px" }}>
              <div style={{ fontSize: 11, color: "var(--muted)", textTransform: "uppercase", marginBottom: 8 }}>{label}</div>
        ...
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, overflow: "hidden" }}>
          ...
          {loading && logs.length === 0 ? (
            ...
            <div style={{ padding: 40, textAlign: "center", color: "var(--muted)" }}>Nenhuma sincronização encontrada para {date}.</div>
          ) : (
