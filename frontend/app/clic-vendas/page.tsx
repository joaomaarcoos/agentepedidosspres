...
function SitBadge({ sit }: { sit?: string }) {
  ...
  const color = s.includes("aprovado") || s.includes("fechado") || s.includes("integrado")
    ? "var(--success)"
    : s.includes("cancel")
      ? "var(--error)"
      : "var(--muted)";
        ...
        >
          <div>
            ...
            <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
              <InfoChip label="Cliente" value={String(pedido.cod_cli || "-")} />
          ...
          {itens.length === 0 ? (
            ...
            >
              ...
              <span style={{ fontSize: 12 }}>
                Esse modulo mostra os pedidos ja persistidos no banco.
              </span>
            </div>
          ) : (
            <>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: "var(--surface2)" }}>
                    {["Codigo", "Produto", "Qtd", "Unid", "Preco Unit.", "Total"].map((header) => (
                      <th
                        key={header}
                        style={{
                          padding: "9px 14px",
                          textAlign: header === "Qtd" || header === "Preco Unit." || header === "Total" ? "right" : "left",
                          fontSize: 10,
                          fontWeight: 700,
                          color: "var(--muted)",
                          textTransform: "uppercase",
                  ...
                  {itens.map((item, idx) => (
                    <tr
                      ...
                      style={{
                        ...
                        background: idx % 2 === 0 ? "transparent" : "var(--surface2)44",
                      }}
                    >
                      <td
                        style={{
                      ...
                      <td style={{ padding: "10px 14px", fontSize: 13, color: "var(--text)", maxWidth: 200 }}>
                        {item.desPro || "-"}
                      </td>
                      <td
                        style={{
                      ...
                      >
                        {item.qtdPed != null ? Number(item.qtdPed).toLocaleString("pt-BR") : "-"}
                      </td>
                      <td
                        style={{
                          padding: "10px 14px",
                      ...
                      >
                        {item.preUni != null
                          ? `R$ ${item.preUni.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`
                          : "-"}
                      </td>
                      <td
                        style={{
                      ...
                      >
                        {item.vlrTotal != null
                          ? `R$ ${item.vlrTotal.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`
                          : "-"}
                      </td>
                    </tr>
                  ))}
        ...
        {itens.length > 0 && (
          ...
          >
            <span style={{ fontSize: 12, color: "var(--muted)" }}>
              ...
              {itens.reduce((sum, item) => sum + (item.qtdPed || 0), 0).toLocaleString("pt-BR")} unidades
            </span>
...
export default function ClicVendasPage() {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      setSyncMsg(`${result.message} A listagem abaixo continua lendo a base local.`);
      await loadPedidos(1);

      const today = new Date().toISOString().slice(0, 10);
      const logs = await clicVendasApi.getSyncLogs(today);
      const last = logs.logs.find((log) => log.status === "success" || log.status === "error");
      setLastLog(last || null);
    } catch (error) {
  ...
  const totalValue = pedidos.reduce((sum, pedido) => sum + (pedido.order_total_value || 0), 0);
  const uniqueClients = new Set(pedidos.map((pedido) => pedido.cod_cli)).size;
  const rangeLabel = dias === 0 ? "Toda a base local" : `Ultimos ${dias} dias`;

  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <Header title="ClicVendas - Pedidos no banco" />
      ...
      <div style={{ flex: 1, overflow: "auto", padding: 28 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 24 }}>
          {[
            ...
            {
              label: "Ultima sync",
              value: lastLog ? new Date(lastLog.triggered_at).toLocaleTimeString("pt-BR") : "-",
              icon: RefreshCw,
              color: lastLog?.status === "success" ? "var(--success)" : "var(--muted)",
            },
        ...
        >
          ...
          >
            <span style={{ fontWeight: 700, fontSize: 13, color: "var(--muted)", textTransform: "uppercase", letterSpacing: 1 }}>
              Pedidos persistidos - {rangeLabel}
            </span>
            <span style={{ fontSize: 12, color: "var(--muted)" }}>
              {total} registros • leitura direta da base local
            </span>
          </div>
          ...
          {loadingPedidos ? (
            ...
            <div style={{ padding: 40, textAlign: "center", color: "var(--muted)" }}>
              Nenhum pedido encontrado no banco para este recorte.
            </div>
          ) : (
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: "var(--surface2)" }}>
                    {["Pedido", "Cliente", "Data", "Situacao", "Valor", "Itens"].map((header) => (
                      <th
                        key={header}
                        style={{
                          padding: "10px 16px",
                  ...
                  {pedidos.map((pedido, index) => {
                    ...
                    return (
                      <tr
                        ...
                        onMouseLeave={(e) => {
                          (e.currentTarget as HTMLElement).style.background = isSelected ? "var(--accent)18" : "transparent";
                        }}
                      >
                        ...
                        <td style={{ padding: "10px 16px" }}>
                          <span
                            style={{
                              ...
                              gap: 5,
                              fontSize: 12,
                              color: (pedido.items_json?.length || 0) > 0 ? "var(--text)" : "var(--muted)",
                              background: (pedido.items_json?.length || 0) > 0 ? "var(--accent)18" : "transparent",
                              padding: "2px 8px",
                              borderRadius: 12,
                              border: (pedido.items_json?.length || 0) > 0 ? "1px solid var(--accent)44" : "none",
                            }}
                          >
                            <Package size={11} />
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button
                  ...
                  style={{
                    background: "var(--surface2)",
                    border: "1px solid var(--border)",
                    color: page >= pages ? "var(--border)" : "var(--text)",
                    borderRadius: 6,
                    padding: "5px 10px",
