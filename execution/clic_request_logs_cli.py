...
def list_logs(
    ...
    db = _db()
    try:
        query = db.table("clic_request_logs").select("*")
        if status:
...
def get_log(log_id: str) -> dict:
    ...
    try:
        rows = db.table("clic_request_logs").select("*").eq("id", log_id).limit(1).execute().data or []
    except Exception:
        rows = db.table("secretary_orders").select("*").eq("id", log_id).limit(1).execute().data or []
        if rows:
            return _secretary_order_to_log(rows[0], detail=True)
        raise
    if not rows:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Logs de requisicoes ao Clic Vendas")
